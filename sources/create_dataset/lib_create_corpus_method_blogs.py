...
def create_corpus_from_blogs_names(file_list_of_blogs, num_articles=0):
	...
	"""
	if(num_articles == 0):
		keep_all_articles = True
	else:
		keep_all_articles = False
	...
	for blog_name in blogs_names:
		...
		articles_urls = get_all_articles_from_blog(blog_name, keep_all_articles, num_articles=num_articles)
...
def create_corpus_csv_from_blogs_names(file_list_of_blogs, input_file_extension, output_file_extension):
	...
	"""
	create_corpus_from_blogs_names(file_list_of_blogs="blogs_philosophy_eng.txt", num_articles=3)
